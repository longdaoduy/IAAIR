...
for identifying bottlenecks in the hybrid search pipeline.
"""
...
class PerformanceMonitor:
    ...
    def start_query_tracking(self, query: str) -> LatencyBreakdown:
    ...
    def record_result_count(self, source: str, count: int):
        """Record number of results from a source."""
        if self.current_breakdown:
    ...
    def finish_query_tracking(self) -> Optional[LatencyBreakdown]:
        """Finish tracking current query and store results."""
        if not self.current_breakdown:
            return None
    ...
    def _log_detailed_breakdown(self, breakdown: LatencyBreakdown):
        ...
        logger.warning(f"  Embedding: {breakdown.embedding_time:.3f}s ({breakdown.embedding_time/breakdown.total_time*100:.1f}%)")
        logger.warning(f"  Vector search: {breakdown.vector_search_time:.3f}s ({breakdown.vector_search_time/breakdown.total_time*100:.1f}%)")
        logger.warning(f"  Graph search: {breakdown.graph_search_time:.3f}s ({breakdown.graph_search_time/breakdown.total_time*100:.1f}%)")
        logger.warning(f"  Fusion: {breakdown.fusion_time:.3f}s ({breakdown.fusion_time/breakdown.total_time*100:.1f}%)")
        logger.warning(f"  Reranking: {breakdown.reranking_time:.3f}s ({breakdown.reranking_time/breakdown.total_time*100:.1f}%)")
        logger.warning(f"  AI response: {breakdown.ai_response_time:.3f}s ({breakdown.ai_response_time/breakdown.total_time*100:.1f}%)")
